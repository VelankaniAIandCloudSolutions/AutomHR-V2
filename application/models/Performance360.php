...
class Performance360 extends CI_Model
{
	...
	*/
	...
	static function get_360_performance($user_id, $year ='', $branch_id = '')
	{
		if($year != "")
		{
			self::$db->where('YEAR(created_date)',$year);
		}
	...
	static function get_competencies($user_id , $year ='')
	{
		if($year != "")
		{
			self::$db->where('YEAR(created_by)',trim($year));
		}
	...
	static function get_360_performance_manager($user_id,  $year ='', $role = '', $branch_id ='')
	{
		if($year != "")
		{
			self::$db->where('YEAR(created_by)',$year);
		}
		if($role != 1)
		{
			self::$db->where('teamlead_id',$user_id);
		}

		if($branch_id  != "" || $branch_id > 0)
		{
			self::$db->join('account_details',"account_details.user_id = competencies.user_id","inner");
			self::$db->where("account_details.branch_id", $branch_id);
		}

		return self::$db->group_by('competencies.user_id')->order_by('competencies.id','ASC')->get('competencies')->result_array();
	}
