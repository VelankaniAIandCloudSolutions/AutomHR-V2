...
class Report extends CI_Model
{
	...
	static function month_amount($year,$month){
		$ci =& get_instance();
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE branch_id='".$ci->session->userdata('branch_id')."' and MONTH(payment_date) = '$month' AND refunded = 'No' AND YEAR(payment_date) = '$year'";
    ...
    static function year_amount($year){
		$ci =& get_instance();
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE branch_id='".$ci->session->userdata('branch_id')."' and refunded = 'No' AND YEAR(payment_date) = '$year'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
    ...
    static function invoiced($year,$month){
		$ci =& get_instance();
    	$query = "SELECT * FROM dgt_invoices WHERE branch_id='".$ci->session->userdata('branch_id')."' and MONTH(date_saved) = '$month' AND YEAR(date_saved) = '$year'";
    ...
    static function total_paid(){
		$ci =& get_instance();
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE branch_id='".$ci->session->userdata('branch_id')."' and refunded = 'No'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
    ...
    static function unbilled_expenses($limit = NULL){
    	self::$db->group_by("client");
        return self::$db->where(array('billable'=>'1','invoiced' => '0'))->get('expenses',$limit)->result();
    }
