...
class All_branches extends MX_Controller
{
	...
	function add_branch()
	{
		if ($_POST) {
			...
			if ($this->form_validation->run() != FALSE) {
				...
				$tmp_weekend_working = $this->input->post('weekend');

				// Initialize the resulting array with null or zero values
				$result = array_fill(0, 10, null);

				foreach ($tmp_weekend_working as $value) {
				    if ($value >= 1 && $value <= 10) {
				...
				$branch_details = array(
					...
					'geo_fencing' => 'disable',
					'weekend_workdays' => json_encode($result),
			...
			if ($this->form_validation->run() != FALSE) {
				...
				if (($branch_id != "") && !empty($_FILES['entity_logo']['name'])) {
					...
					if (empty($logo_response['error'])) {
						...
						$logo_response['error'] .='Accepting only jpg/jpeg format';
						$this->session->set_flashdata('tokbox_error', $logo_response['error']);
						redirect('all_branches');
					}
				}
	...
	public function branch_details()
	{
		...
		if (isset($_POST['entity_id']) && $_POST['entity_id'] != '') {
			...
			$this->db->where("branch_id", $_POST['entity_id']);
			$this->db->where("branch_status", "0");
			$resposne = $this->db->get()->row_array();
			if (!empty($resposne)) {
				$output['status'] = true;
				$output['result'] = $resposne['address'];
			}
		}
