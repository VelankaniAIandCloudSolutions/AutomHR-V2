...
class Candidate_chats extends MX_Controller {
...

	{
		...
		$this->load->model(array('candidate_chats_model', 'App'));
...

	{
		...
		if($this->tank_auth->is_logged_in()) {
			...
			if($this->session->userdata('candidate_id')){
				$log_check = $this->candidate_chats_model->candidate_checkById($log_id);

			}else{
				$log_check = $this->candidate_chats_model->user_checkById($log_id);
			}
			...
			if($log_check['session_id'] == '')
			{
				...
				if($this->session->userdata('candidate_id')){
					$this->db->update('registered_candidates',$res);
				}else{
			...
			$data['chat_history'] = $this->candidate_chats_model->user_chat_history($this->tank_auth->get_user_id());

			$data['group_chat_list'] = $this->candidate_chats_model->getChatList($log_id,'group');

			
			if($this->input->get('id')){
				
				$data['single_chat_list'] = $this->candidate_chats_model->getChatuser($this->input->get('id'));
				//echo $this->db->last_query();exit; 
			}else{
				$data['single_chat_list'] = $this->candidate_chats_model->getChatList($log_id,'one');
			}
			...
			$this->template
				 ...
				 ->build('candidate_chats',isset($data) ? $data : NULL);
...
echo json_encode(array('hidden_group_name'=>ucfirst($hidden_group_name),'group_name'=>$_POST['group_name'],'new_group_name'=>ucfirst($new_group_name)));
	...
	public function create_share(){
		...
		if($count!=0){
			...
			$this->db->insert('dgt_candidate_chat_group_details',$da);
			$group_id = $this->db->insert_id();
			...
			for ($i=0; $i <count($member) ; $i++) { 
				...
				if($user){
					...
					$this->db->insert('dgt_candidate_chat_group_members',$datat);	
	...
	public function get_call_notification(){
		...
		if($this->session->userdata('candidate_id')){
			$data = $this->db->get_where('dgt_registered_candidates',$wh)->row_array();
		}else{
		...
		$data = $this->db->get_where('dgt_candidate_call_notification',$where)->row_array();
		// print_r($data); exit;
		if(!empty($data)){	
			...
			if($data['type'] == 'one'){
				...
				$response = $this->db->get_where('candidate_chat_group_details',$wheres)->row_array();			
	...
	public function get_dummy_datas(){
		...
		if(!empty($this->session->userdata('dummy_session_id'))){
			...
			if($this->session->userdata('candidate_id')){
				$this->db->update('dgt_registered_candidates',array('call_status'=>1,'group_id'=>$group_id),array('id'=>$this->login_id));
			}else{
	...
	// echo $_POST['group_id']; exit;
			$where = array('dgt_candidate_chat_group_details.group_id' => $_POST['group_id'],'dgt_candidate_chat_group_members.login_id'=>$this->login_id);
			$data = $this->db->select('dgt_candidate_chat_group_details.session_id,dgt_candidate_chat_group_members.token,dgt_candidate_chat_group_details.group_id')
			->join('dgt_candidate_chat_group_members dgt_candidate_chat_group_members','dgt_candidate_chat_group_members.group_id = dgt_candidate_chat_group_details.group_id')
			->get_where('dgt_candidate_chat_group_details',$where)
			->row_array();
		// echo $this->db->last_query(); exit;
			...
			if(!empty($data)){
				...
				$this->db->update('dgt_candidate_chat_group_members',array('token'=>$my_token),$where);	
		...
		$this->db->delete('dgt_candidate_call_notification',array('group_id' => $_POST['group_id']));
	...
	public function discard_notify(){
		echo $this->db->delete('dgt_candidate_call_notification',array('group_id' => $_POST['group_id']));

		if($this->session->userdata('candidate_id')){
			$where = array('id' =>$this->login_id);
			$data = array('call_status' =>0);
			$this->db->update('dgt_registered_candidates',$data,$where);
		}else{
			$where = array('login_id' =>$this->login_id);
			$data = array('call_status' =>0);
			$this->db->update('dgt_users',$data,$where);
...
// print_r($user); exit;
		...
		$user = $this->db
		->get_where('dgt_registered_candidates',array('id'=>$session_chat_id))
		->row_array();
		...
		if($user['online_status']==0){ 
			echo json_encode(array('error' => $user['first_name'].' is offline'));
			exit;
		}elseif($user['call_status']==1){ 
			echo json_encode(array('error' => $user['first_name'].' already in a call'));
			exit;
		}	
		...
		$count = $this->db->get_where('dgt_candidate_chat_group_details',array('status'=>1,'group_id'=>$group_id))->num_rows();
		//echo $group_id;
		if($count == 0){
			echo json_encode(array('error'=>'No group'));
			exit;
			...
			$this->db->insert('dgt_candidate_chat_group_details',$datas);
			$group_id = $this->db->insert_id();
			...
			$this->db->insert('dgt_candidate_chat_group_members',$dats);
			...
			$this->db->insert('dgt_candidate_call_notification',$notify);
		...
		// echo $this->session->userdata('session_group_id'); exit;
			...
			$sessionId = $this->db->get_where('dgt_candidate_chat_group_details',array('group_id'=>$group_id))->row()->session_id;
...
// echo $connectionMetaData; exit;
			...
			$members = $this->db							
							->join('dgt_users','dgt_users.id = dgt_candidate_chat_group_members.login_id')
							->get_where('dgt_candidate_chat_group_members',array('dgt_candidate_chat_group_members.group_id'=>$group_id,'dgt_candidate_chat_group_members.login_id !=' =>$this->login_id))
							->result_array();
  ...
  // exit;
	...
	public function online_offline()
	{
		...
		$chats = $this->candidate_chats_model->get_chat_messagesbyID($to_id,'one');
		// print_r($chats); exit;
	...
	public function get_message_token()
	{
		...
		$to_connection_details = $this->candidate_chats_model->check_connections($to_id);
		$from_connection_details = $this->candidate_chats_model->check_connections($this->login_id);

		$common_session = $this->candidate_chats_model->get_common_session();
	...
	public function message_saving()
	{
		...
		if(($from_id != 0) && ($to_id != 0) && ($connection_id != '') && ($msg != '')){
			$from_check_connection = $this->candidate_chats_model->check_connections($from_id);
	...
	public function get_chat_msg_notification()
	{
		...
		$group_details = $this->candidate_chats_model->get_groupsByUserId($this->login_id);
	...
	public function chat_single_history()
	{
		...
		$msg_details = $this->candidate_chats_model->chat_usersignle_history($user_id,$log_id);
	...
	public function chat_single_delete()
	{
		...
		$res = $this->db->delete('dgt_candidate_chat_conversations');
		echo "success"; exit;
	}
	...
	public function group_chat_userlist()
	{
		...
		$msg_details = $this->candidate_chats_model->chat_usergroup_history($group_id);
		$all_group_members = $this->candidate_chats_model->chatgroup_members_list($group_id);
	...
	public function grouplist_receiver()
	{
		...
		$all_groups = $this->candidate_chats_model->chatgroup_members_list($group_id);
		$receivers = array();
