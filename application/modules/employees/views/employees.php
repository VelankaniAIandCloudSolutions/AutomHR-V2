	...
	<div class="row filter-row">
		...
		<div class="col-sm-6 col-xs-12 col-md-3">  
			...
			<div class="form-group form-focus">
				...
				<select class="select floating form-control" id="entity_id" name="entity_id" style="padding: 14px 9px 0px;"> 

					<option value="" selected="selected">All Entity</option>
					<?php if(!empty($branches)){ 
							 foreach ($branches as $branches_value) { ?>
							<option value="<?php echo $branches_value['branch_id']; ?>" <?php if(!empty($_POST['entity_id']) && $_POST['entity_id'] == $branches_value['branch_id']){echo 'selected';}?>><?php echo $branches_value['branch_name']; ?></option>
					<?php } } ?>
				</select>
			</div>
		</div>
...
<table id="table-employee" class="table table-striped custom-table m-b-0 dataTable no-footer">
	...
	<tr>
		...
		<th><?=lang('name')?></th>
		<th><?=lang('razorpay_xpayroll_resposne')?></th>
...
<script>
	$(document).ready(function() {
		...
		"ajax": "<?php echo base_url().'employees/employees_list';?>?employee_id="+employee_id+"&username="+username+"&department_id="+department_id+"&employee_email="+employee_email+"&entity_id="+entity_id
		} );
	} );
</script>
