<div class="content">
	<div class="row">
		...
		<div class="col-sm-8 col-xs-9 text-right m-b-0">
			<?php 
			//if(User::is_staff() ) { ?> 
			<?php if(App::is_permit('menu_expenses','write')){?>
		...
		<table id="table-expenses" class="table table-striped custom-table m-b-0">
			<thead>
				<tr>
					...
					<?php if(User::is_admin() || App::is_permit('menu_expenses','read')) { ?> 
			...
			<?php foreach ($expenses as $key => $e) { 
				...
				$records_added_by_self =  $this->db->get()->result();


				$this->db->select("*");
				$this->db->from("expense_approvers");
				$this->db->where("expense_approvers.approvers", $this->session->userdata('user_id'));
				$this->db->where("expense",$e->id);
				$this->db->where("status = 0");
				$this->db->order_by("id","asc");
				$record_pending =  $this->db->get()->result();

				$this->db->select("*");
				$this->db->from("expense_approvers");
				$this->db->where("expense",$e->id);
				// $this->db->order_by("id","desc");
				$approval_list =  $this->db->get()->result();
			...
			<tr id="row_id_<?=$e->id?>" >
				...
				<td class="col-currency">
					...
					if(!empty($p->currency))
					{
						$cur = ($p != NULL) ? $p->currency : config_item('default_currency'); 

						$cur = ($e->client > 0) ? Client::client_currency($e->client)->code : $cur;
					}
					else{
						$cur = ($p != NULL) ? $p->currency : 'INR'; 
						$cur = ($e->client > 0) ? Client::client_currency($e->client)->code : $cur;
					}
				...
				<td>
				    ...
				    $approved_status = 'Pending';
				    $label_color = 'warning';
				    if(sizeof($approval_list) > 0)    
				    {
				        $approval_count = 0;
				        foreach($approval_list as $approval_list_val)
				        {
				            $approval_count++;

				            if($approval_list_val->status == '1')
				            {
				                $approved_status = 'Approved';
				                $label_color = 'success';
				            }
				            elseif($approval_list_val->status == '2')
				            {
				                $approved_status = 'Declined';
				                $label_color = 'danger';
				            }
				            ...
				            echo 'Approval '.$approval_count.' <span class="small label label-' . $label_color . '">' . $approved_status . '</span><br>';
				        }
				    }                     
				    ?>
				</td>
				...
				<?php if(User::is_admin() || App::is_permit('menu_expenses','read')) { 
					...
					if(sizeof($records_added_by_self) == 1 || count($record_pending) == 0)
					{
						echo '<td></td>';
					} 
					else{
