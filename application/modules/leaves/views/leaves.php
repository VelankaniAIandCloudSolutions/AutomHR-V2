...
<div class="content">
			...
			<div class="row filter-row" id="search_form_div">
					...
					<div class="form-group form-focus select-focus">
						...
						<select class="select2-option floating form-control" id="ser_leave_type" name="ser_leave_type">
							...
							<?php for ($i = 0; $i < count($leav_types); $i++) { ?>
								<option value="<?php echo $leav_types[$i]['leave_type_id']; ?>" <?php if (!empty($_POST['ser_leave_type']) && $_POST['ser_leave_type'] == $leav_types[$i]['leave_type_id']) {
																									echo 'selected';
																								} ?>><?= $leav_types[$i]['leave_type'] ?></option>
							<?php } ?>
						</select>
				...
				<div class="col-xs-6 col-sm-4 col-md-4 col-lg-2">
					<div class="form-group form-focus select-focus">
						...
						<select class="select2-option floating form-control" id="ser_leave_sts" name="ser_leave_sts">
							<option value="" <?php if (empty($_POST['ser_leave_sts']) || $_POST['ser_leave_sts'] == '') {
													echo 'selected';
												} ?>> All Status </option>
							<option value="0" <?php if (isset($_POST['ser_leave_sts']) && $_POST['ser_leave_sts'] != '' && $_POST['ser_leave_sts'] == 0) {
													echo 'selected';
												} ?>> Pending </option>
							<option value="1" <?php if (!empty($_POST['ser_leave_sts']) && $_POST['ser_leave_sts'] == 1) {
													echo 'selected';
												} ?>> Approved </option>
							<option value="2" <?php if (!empty($_POST['ser_leave_sts']) && $_POST['ser_leave_sts'] == 2) {
													echo 'selected';
												} ?>> Rejected </option>
						</select>
				...
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-2">
					<div class="form-group form-focus">
						...
						<input type="text" class="form-control floating" id="ser_leave_user_name" name="ser_leave_user_name" value="<?php if (!empty($_POST['ser_leave_user_name'])) {
																																		echo $_POST['ser_leave_user_name'];
																																	} ?>">
					...
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 p-0">
						<div class="form-group form-focus">
							<div class="ref-icon">
								...
								<input class="form-control floating leaves-datepicker" readonly type="text" data-date-format="dd-mm-yyyy" id="ser_leave_date_from" value="<?php if (!empty($_POST['ser_leave_date_from'])) {
																																												echo $_POST['ser_leave_date_from'];
																																											} ?>" size="16" name="ser_leave_date_from">
					...
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 p-0">
						<div class="form-group form-focus">
							<div class="ref-icon">
								...
								<input class="form-control floating leaves-datepicker leaves-to-datepicker" readonly type="text" data-date-format="dd-mm-yyyy" id="ser_leave_date_to" value="<?php if (!empty($_POST['ser_leave_date_to'])) {
																																																	echo $_POST['ser_leave_date_to'];
																																																} ?>" size="16" name="ser_leave_date_to">
							...
							if (!empty($leave_list)) {
								foreach ($leave_list as $key => $levs) {  ?>
									...
									<tr>
										...
										<td class="text-right">
											<a class="btn btn-success btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/approve_request/management/<?= $levs['id'] ?>" title="Approve" data-original-title="Approve">
												<i class="fa fa-thumbs-o-up"></i>
											</a>

											<a class="btn btn-danger btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/reject_request/management/<?= $levs['id'] ?>" title="Reject" data-original-title="Reject">
	...
	?>
		...
		<div class="panel panel-white new_leave_reqst" style="display:none">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-lg-4">
						<input class="form-control " readonly size="16" type="text" onchange="leave_days_calc();" value="" name="req_leave_date_from" id="req_leave_date_from" data-date-format="dd-mm-yyyy">

						<input class="form-control req_leave_date_from" readonly size="16" type="text" name="req_leave_date_from" style="display: none;">
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-2 control-label">To <span class="text-danger">*</span></label>
					<div class="col-lg-4">
						<input class=" form-control " readonly size="16" type="text" onchange="leave_days_calc();" value="" name="req_leave_date_to" id="req_leave_date_to" data-date-format="dd-mm-yyyy">
						<input class="req_leave_date_to form-control " readonly size="16" type="text" name="req_leave_date_to" style="display: none;">
				...
				<div class="table-responsive">
					...
					<table class="table table-striped custom-table m-b-0">
						...
						<tbody>
										...
										<td>
											<?php if ($check_teamlead['is_teamlead'] == 'no') { ?>
												<?php if ($levs['status'] == 0) { ?>
													<a class="btn btn-danger btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/compen_cancel/<?= $levs['id'] ?>" title="Cancel" data-original-title="Cancel">
											...
											if ($check_teamlead['is_teamlead'] == 'yes') {
												...
												?>
													<a class="btn btn-success btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/approve_request/teamlead/<?= $levs['id'] ?>" title="Approve" data-original-title="Approve">
														<i class="fa fa-thumbs-o-up"></i>
													</a>
													<a class="btn btn-danger btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/reject_request/teamlead/<?= $levs['id'] ?>" title="Reject" data-original-title="Reject">
														<i class="fa fa-thumbs-o-down"></i>
													</a>
											<?php }
											} ?>
							...
							<?php  } else { ?>
								<tr>
									<td colspan="9">No details were found</td>
								</tr>
							<?php } ?>
						</tbody>
					</table>
		...
		<form action="<?php echo base_url("leaves/bulk_leave_approve");?>" method="post">
										...
										<tr>
											...
											<td>
												<?php if (empty($check_teamlead['is_teamlead']) || $check_teamlead['is_teamlead'] != 'yes') { ?>
													<?php if ($levs['status'] == 0) { ?>
														<a class="btn btn-danger btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/cancel/<?= $levs['id'] ?>" title="Cancel" data-original-title="Cancel">
															<i class="fa fa-times"></i>
														</a>
													<?php }
												}
												if ($check_teamlead['is_teamlead'] == 'yes') {
													if (($levs['status'] != 3) &&  ($levs['status'] != 1)) {
													?>
														<a class="btn btn-success btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/approve/teamlead/<?= $levs['id'] ?>" title="Approve" data-original-title="Approve">
															<i class="fa fa-thumbs-o-up"></i>
														</a>
														<a class="btn btn-danger btn-xs" data-toggle="ajaxModal" href="<?= base_url() ?>leaves/reject/teamlead/<?= $levs['id'] ?>" title="Reject" data-original-title="Reject">
															<i class="fa fa-thumbs-o-down"></i>
														</a>
												<?php }
												else{ echo "-";}
			...
			<input type="submit" value="Approve" class="btn btn-primary" id="bulk_leave_btn">
		</form>
		<!-- user leave end -->
	<?php } ?>
</div>
...
<script>
	...
	function Getholidaydates() {
		...
		$.post(base_url + 'leaves/Getholidaydates/', {
