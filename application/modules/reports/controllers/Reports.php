	...
	function _payslip_report(){
		...
		// exit;
			...
			if (in_array(''.date('Y-m-d', $time).'', $my_val)) {
				...
				$this->db->where('DATE(leave_from)<=',$chdate);
				$this->db->where('DATE(leave_to)>=', $chdate); 
					...
					foreach ($results as $rows) {
						...
						for($d=1; $d<=$number; $d++)
						{
							...
							if(!empty($rows['month_days'])){
								...
								if(sizeof($month_days_in_out[$a_day-1]) > 1)
								{
									
									$tmp_checkin_count = 0;
									$tmp_checkin_count = sizeof($month_days_in_out[$a_day-1]);
									
									$tmp_checkin = $month_days_in_out[$a_day-1][$tmp_checkin_count-1]['punch_in'];
									$tmp_checkout = $month_days_in_out[$a_day-1][$tmp_checkin_count-1]['punch_out'];

									$month_days_in_out[$a_day-1] = array();
									$month_days_in_out[$a_day-1][0]['day'] = '1'; 
									$month_days_in_out[$a_day-1][0]['punch_in'] = $tmp_checkin;
									$month_days_in_out[$a_day-1][0]['punch_out'] = $tmp_checkout;
								}
