	...
	function account_list(){
        ...
        foreach ($list as $key => $user) {
        	...
        	$emp_code = '';
         	...
         	$role_details = $this->db->get_where('roles',array('r_id'=>$user['user_type']))->row_array();
         	$branch_details = $this->db->get_where('branches',array('branch_id'=>$branch_id))->row_array();
            $no++;
            $row = array();
            $row[] = '<td class="sorting_1">
            		  <span class="avatar">'.ucfirst($user['fullname'][0]).'</span>
            		  <h2>'.ucfirst($user['fullname']).$emp_code.

            		  ' <span>'.ucfirst($branch_details['branch_name']).'</span></h2>
            		  </td>';
			$row[] = '<td>'.$user['email'].'</td>';
			$row[] = '<td>'.ucfirst($branch_details['branch_name']).'</td>';
			if($user['created'] !="")
			{
				$row[] = '<td>'.date('d M Y',strtotime($user['created'])).'</td>';
			} 
			else{
				$row[] = '<td>'.date('d M Y',strtotime($user['modified'])).'</td>';
			}
