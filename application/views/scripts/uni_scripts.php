...
?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#table-tickets").dataTable().fnSort([
                [0, 'desc']
            ]);
            $("#table-tickets-archive").dataTable().fnSort([
                [1, 'desc']
            ]);


            $("#table-projects-client").dataTable().fnSort([
                [4, 'asc']
            ]);
            $("#table-projects-archive").dataTable().fnSort([
                [5, 'desc']
            ]);
            $("#table-teams").dataTable().fnSort([
                [0, 'asc']
            ]);
            $("#table-milestones").dataTable().fnSort([
                [2, 'desc']
            ]);
            $("#table-milestone").dataTable().fnSort([
                [2, 'desc']
            ]);
            $("#table-tasks").dataTable().fnSort([
                [2, 'desc']
            ]);
            $("#table-files").dataTable().fnSort([
                [2, 'desc']
            ]);
            $("#table-links").dataTable().fnSort([
                [0, 'asc']
            ]);
            $("#table-project-timelog").dataTable().fnSort([
                [0, 'desc']
            ]);
            $("#table-tasks-timelog").dataTable().fnSort([
                [0, 'desc']
            ]);

            $("#table-clients").dataTable().fnSort([
                [0, 'asc']
            ]);
            ...
            $("#table-client-details-1").dataTable().fnSort([
                [1, 'asc']
            ]);
            $("#table-client-details-2").dataTable().fnSort([
                [2, 'desc']
            ]);
            $("#table-client-details-3").dataTable().fnSort([
                [0, 'asc']
            ]);
            $("#table-client-details-4").dataTable().fnSort([
                [1, 'asc']
            ]);
            $("#table-templates-1").dataTable().fnSort([
                [0, 'asc']
            ]);
            $("#table-templates-2").dataTable().fnSort([
                [0, 'asc']
            ]);
            ...
            $("#table-bugs").dataTable().fnSort([
                [1, 'desc']
            ]);
            $("#table-stuff").dataTable().fnSort([
                [0, 'asc']
            ]);
            $("#table-activities").dataTable().fnSort([
                [0, 'desc']
            ]);
...
<?php if (isset($sortable)) { ?>
    ...
    <script type="text/javascript">
        ...
        $('#menu-admin').sortable({
            ...
            afterMove: function() {
                clearTimeout(t2);
                t2 = setTimeout('saveMenu(\'admin\',1)', 500);
            }
        });
...
<?php if (isset($nouislider)) { ?>
    ...
    <script type="text/javascript">
        ...
        function edit_model(task_id) {
            ...
            $.post(base_url + 'all_tasks/get_task_details/', {
                task_id: task_id
            }, function(datas) {
    ...
    $(document).ready(function() {
        ...
        /*$('#admin_search_leave').on('click', function (e) {
        		...
        		$.ajax({
                    ...
                    type   : "POST",
                    data   : {l_entity:l_entity,l_type:l_type,l_sts:l_sts,uname:uname,dfrom:dfrom,dto:dto,'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
                    success: function(response) {
        ...
        $(".gantt").gantt({
            source: [
                ...
                foreach ($tasks as $key => $t) {
                    $start_date = ($t->start_date == NULL) ? $t->date_added : $t->start_date; ?> {
                        "name": '<a href="<?= site_url() ?>projects/view/<?= $project ?>?group=tasks&view=task&id=<?= $t->t_id ?>" class="text-info"><?= addslashes($t->task_name) ?> </a>',
                        "desc": "",
                        "values": [{
                            "from": Date.parse("<?= date('Y/m/d', strtotime($start_date)) ?>"),
                            "to": Date.parse("<?= date('Y/m/d', strtotime($t->due_date)) ?>"),
                            ...
                            "customClass": '<?php if ($t->task_progress == '100') {
                                                echo "ganttGreen";
                                            } else {
                                                echo "ganttRed";
                                            } ?>',
...
<script>
    ...
    function single_chat_histroy() {
        var value = 1;
        $.post(base_url + 'chats/chat_single_history', {
            current_chat: value
        }, function(res) {
    ...
    function group_chat_histroy() {
        var group_id = "";
        $.post(base_url + 'chats/group_chat_userlist', {
            group_id: group_id
        }, function(res) {
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                $.each(res.chats, function(key, value) {
                    ...
                    var tot_new = 0;
                    $.each(value.html2, function(key2, value2) {
                        tot_new++;
                    });
...

    {
        ...
        $(ele).parent().children().each(function(index, element) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('.chat-wrap-inner').animate({
                            scrollTop: $('#chat_details_appnd').outerHeight()
                        }, 1000);
                        if ($('#chat-' + chat_id).length == 1) {

                            var d = $('#chat-' + chat_id).find('.panel-body');

                            $(d).append(response.html2).animate({
                                scrollTop: $(d)[0].scrollHeight
                            }, 1000);
        ...
        if (chat_content.length > 0) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (chat_tbl_id == 0) {
                        $('#chat_tbl_id_bx' + chat_id).val(response.chat_tbl_id);
                    }
    ...
    $('#attendance_search_btn').click(function() {
        ...
        if (attendance_date != '') {
            $.post('<?= base_url() ?>attendance/attendance/get_list', {
                user_id: user_id,
    ...
    $(document).on('change', '#department_name,#editdepartment_name', function() {
        ...
        if (department != '') {
            $.post('<?= base_url() ?>employees/designations', {
            ...
            // $.post(base_url+'employees/teamlead_options/',{dept_id:department},function(res){
    ...
    $(document).on('change', '#company_name', function() {
        ...
        if (company != '') {
            $.post('<?= base_url() ?>reports/employees', {
                company: company
            }, function(data) {
                if (data) {
                    var full_records = JSON.parse(data);
    ...
    $(document).ready(function() {
        $('.GroupChatList').on("click", function() {
            ...
            $.post(base_url + 'chats/group_chat_userlist', {
                ...
                // console.log(res);
                $.post(base_url + 'chats/group_user_grouplist', {
                    group_id: group_id
                }, function(data) {
                ...
                $.post(base_url + 'chats/grouplist_receiver', {
                    group_id: group_id
                }, function(result) {
                    // console.log(result);
        ...
        $(document).on("click", '.SingleChatList', function() {
            ...
            $.post(base_url + 'chats/online_offline', {
                to_id: user_id
            }, function(res) {
                if (res == 'offline') {
        ...
        $(document).on("click", '.DeleteChatMsgg', function() {
            ...
            if (res == true) {
                ...
                var user_id = $(this).data('userid');
                $.post(base_url + 'chats/chat_single_delete', {
                    msg_id: msg_id
                }, function(res) {
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.clone_invoice_btn').click(function() {
            ...
            $.post(base_url + 'invoices/invoice_cloning/', {
                invoice_id: invoice_id
            }, function(res) {
                setInterval(function() {
                    toastr.success('Invoice Copied');
...
<?php if ($this->uri->segment(1) == 'organisation') { ?>
    <script>
        $('#department').change(function() {
            ...
            $.post(base_url + 'auth/designation_option/', {
                dept_id: dept_id
            }, function(res) {
                var leads_name = JSON.parse(res);
...
<script type="text/javascript">
    ...
    function delete_assigne(project_id, task_id) {
        $.post('<?php echo base_url(); ?>all_tasks/delete_assigne', {
            'task_id': task_id
        }, function(data) {
    ...
    function delete_due_date(project_id, task_id) {
        $.post('<?php echo base_url(); ?>all_tasks/delete_due_date', {
            'task_id': task_id
        }, function(data) {
    ...
    function task_description(task_id) {
        ...
        if (description != '') {
            $.post('<?php echo base_url(); ?>all_tasks/description_update', {
                'description': description,
                'task_id': task_id
            }, function(data) {
    ...
    function task_name(task_id, project_id) {
        ...
        if (task_name != '') {
            $.post('<?php echo base_url(); ?>all_tasks/task_name_update', {
                'task_name': task_name,
                'task_id': task_id,
    ...
    function task_progress(task_id) {
        ...
        if (progress != '') {
            $.post('<?php echo base_url(); ?>all_tasks/progress_update', {
                'progress': progress,
                'task_id': task_id
            }, function(data) {
        ...
        function edit_policies(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.post('<?php echo base_url(); ?>policies/policies_details', {
                        id: id
                    }, function(data) {
                        $('.upload_details').show();
        ...
        function edit_resignation(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var option = '<option value="' + data.employee + '">' + data.fullname + '</option>';
                    ...
                    if (data.designation_id != '') {
						$('#designation_id option').each(function() {
							if ($(this).val() === data.designation_id) {
								$(this).prop('selected', true);
							}
						});
					}
                    ...
                    $("#exit_request_submit_date").val(data.exit_request_submit_date);
                    $("#requested_last_working_date").val(data.requested_last_working_date);
                    $("#notice_period_end_date").val(data.notice_period_end_date);
                    $("#elaborate_reason").val(data.elaborate_reason);
                    $("#personal_email").val(data.personal_email);
                    if (data.resignation_status != '') {
						$('#resignation_status option').each(function() {
							if ($(this).val() === data.resignation_status) {
								$(this).prop('selected', true);
							}
						});
					}

                    let all_attachments = data.attachments;
                    let attachmentDiv = document.getElementById("attachment_files");

                    for (let j = 0; j < all_attachments.length; j++) {
                        let attachmentLink = document.createElement("a");
                        let url = '';
                        url = '<?php echo base_url("uploads/resignation_attachment/")?>'+data.employee;
                        attachmentLink.href = url+'/'+all_attachments[j].name; 
                        attachmentLink.textContent = all_attachments[j].name; 
                        attachmentLink.target = "_blank"; 

                        let lineBreak = document.createElement("br"); 
                        attachmentDiv.appendChild(attachmentLink);
                        attachmentDiv.appendChild(lineBreak); 
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    alert('Error get data from ajax');
    ...
    <script type="text/javascript">
        $(document).ready(function(e) {
            $("#add_terminations").on('submit', (function(e) {
                ...
                var fileUpload = document.getElementById("terminationletter");
                ...
                /*if (!regex.test(fileUpload.value.toLowerCase())) {
                	lblError = "Termination Letter having extensions: <b>" + allowedFiles.join(', ') + "</b> only.";
                	toastr.error(lblError);
                	return false;
                }*/
...
<?php if ($this->uri->segment(1) == 'promotion') { ?>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#employee_id').change(function() {
                ...
                $.post('<?php echo base_url(); ?>promotion/get_departments', {
    ...
    <script type="text/javascript">
        ...
        function add_promotion() {
            ...
            $("#employee_id1").append($newOption).trigger('change');
            $("#employee_id").select2();
            $('#employee_id').select2('destroy');
            $("#employee_id").select2();
        ...
        function send_promotion() {
            $('#send_promotion').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Send Promotion'); // Set title to Bootstrap modal title
            return false;
        }

        function accept_promotion() {

            var form = new FormData(document.getElementById("add_promotions"));
        ...
        function edit_promotion(id) {
            ...
            $.ajax({
                ...
                success: function(datas) {
                    ...
                    $('[name="currentsalary"]').val(datas.currentsalary);
                    $('[name="newsalary"]').val(datas.newsalary);
                    $('[name="effectivedate"]').val(datas.effectivedate);
                    let department = $('[name="department"]').val(datas.department);
                    // alert(datas.department);return false;
                    // $('#cur_prm_to').val(datas.promotionto);

                    promotion_department_edit(datas.department, datas.promotionto);
                    // $('[name="promotionto"]').val(datas.promotionto);
                    var $newOption = $("<option selected='selected'></option>").val(datas.employee).text(datas.emp_name);
                    $("#employee_id1").append($newOption).trigger('change');
        ...
        function promotion_department_edit(deptid, des_id) {
            // alert(deptid);

            $('.promotion').empty().trigger("change");

            $.post('<?php echo base_url(); ?>promotion/get_designation_by_dept', {
                deptid: deptid
            }, function(data) {

                var obj = jQuery.parseJSON(data);

                $('.promotion').select2('destroy');
                $(".promotion").select2();
                $('.promotion').find("option:eq(0)").html("--Select--");
                ...
                $('[name="promotionto"]').val(des_id).trigger("change");
        ...
        function accept_promotion_new(id)

        {

            $.ajax({
                url: "<?php echo site_url('promotion/promotion_accept') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
        ...
        function reject_promotion(id)

        {

            $.ajax({
                url: "<?php echo site_url('promotion/promotion_reject') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
    ...
    <script type="text/javascript">
        $(document).ready(function(e) {
            $("#add_promotions").on('submit', (function(e) {
                ...
                var promotionletter = $('#promotionletter').val();


                var allowedFiles = [".doc", ".docx", ".pdf"];
                var fileUpload = document.getElementById("promotionletter");
                var regex = new RegExp("([a-zA-Z0-9\s_\\.\-:])+(" + allowedFiles.join('|') + ")$");
                ...
                if (currentsalary == '') {
                    toastr.error('Current Salary  Field is Required');
                    return false;
                }
                if (newsalary == '') {
                    toastr.error('New Salary  Field is Required');
                    return false;
                }
                if (effectivedate == '') {
                    toastr.error('Effective Date  Field is Required');
                    return false;
                }
                if (promotionletter == '') {
                    toastr.error('Promotion Letter  Field is Required');
                    return false;
                }
                /*if (!regex.test(fileUpload.value.toLowerCase())) {
                	lblError = "Promotion Letter having extensions: <b>" + allowedFiles.join(', ') + "</b> only.";
                	toastr.error(lblError);
                	return false;
                }*/
                send_promotion();
...
<script type="text/javascript">
    function user_status_change(id) {
        ...
        $.post('<?php echo base_url(); ?>payroll/status_change', {
            employeeid: id,
            status: status
        }, function(data) {
            if (data == 1) {
    ...
    function send_offer_letter_mail() {
        let ascid = $('#ch_asso_id_preview').val();
        let ch_asso_id_img = $('#ch_asso_id_img').val();
        console.log(ascid);
        bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    ...
                    $.ajax({
                        url: 'offers/send_offer_candidate/',
                        dataType: 'json',
                        type: 'POST',
                        data: {
                            'assoc_id': ascid,
                            'ch_asso_id_img': ch_asso_id_img
                        },
                        success: function(data) {
    ...
    function send_Appmails(ascid) {
        $('#digital_signature_div').modal('show');
        $('#ch_asso_id').val(ascid);
    }

    function accept_offer(candidate_id, job_id) {
        $('#digital_signature_div').modal('show');
        $('#ch_candidate_id').val(candidate_id);
        $('#ch_job_id').val(job_id);
    }

    function can_accept_offer_letter() {
        let ascid = $('#ch_asso_id_preview').val();
        let ch_asso_id_img = $('#ch_asso_id_img').val();
        let job_id = $('#ch_job_id').val();
        let can_id = $('#ch_candidate_id').val();
        console.log(ascid);
        bootbox.confirm({
            message: "Are you sure, accept this offer?",
            buttons: {
                confirm: {
                    label: 'Yes',
                    className: 'btn-success'
            ...
            callback: function(result) {
                if (result) {
                    ...
                    $.ajax({
                        url: 'candidates/send_offer_candidate/',
                        dataType: 'json',
                        type: 'POST',
                        data: {
                            'job_id': job_id,
                        ...
                        success: function(data) {
                            ...
                            if (data.response == 'ok') {
                                window.location.href = base_url + 'candidates/user_offer_status_change/8/' + can_id + '/' + job_id;
                            }
                        },
    ...
    function appcandmails_preview() {
        let job_id = $('#ch_job_id').val();
        let can_id = $('#ch_candidate_id').val();
        $('#prev_candidate_id').val(can_id);
        $('#prev_job_id').val(job_id);
        let sign = $('#signature64').val();
        $('#sign_error').hide();
        let sign_type = $('input[name="signature_type"]:checked').val();
        let sign_image = $('#sign_image_file').val();
        if (sign_type == 'digital_signature') {
            if (sign == '') {
                $('#sign_error').show();
                return false;
            }
            send_sign_preview(job_id, can_id, sign);
        } else {
            // Allowing file type
            ...
            if (sign_image == '') {
                ...
                $('#sign_img_error').show();
                return false;
            } else if (!allowedExtensions.exec(sign_image)) {
                $('.eror_sign').hide();
                $('#sign_img_val_error').show();
                return false;
            }
            send_upload_preview(job_id, can_id);
        }

    }

    function send_upload_preview(job_id, can_id) {
        var file_data = $('#sign_image_file').prop('files')[0];
        var form_data = new FormData();
        form_data.append('file', file_data);
        form_data.append('job_id', job_id);
        form_data.append('can_id', can_id);
        $.ajax({
            url: base_url + 'candidates/send_applicantmail_preview/',
            type: "post",
            data: form_data,
            processData: false,
            contentType: false,
            ...
            success: function(data) {
                var obj = jQuery.parseJSON(data);
                $('#digital_signature_div').modal('hide');
                $('#ch_asso_id_img').val(obj.img);
                $('#ch_asso_id_preview').val(obj.offer_id);
                $('#preview_div').html(obj.content);
                $('#digital_signature_preview').modal('show');
            }
        });
    }

    function send_sign_preview(job_id, can_id, sign) {
        $.ajax({
            url: base_url + 'candidates/send_applicantmail_preview/',
            dataType: 'json',
            type: 'POST',
            ...
            success: function(data) {
                if (data.response == 'ok') {
                    $('#digital_signature_div').modal('hide');
                    $('#ch_asso_id_img').val(data.img);
                    $('#ch_asso_id_preview').val(data.offer_id);
                    $('#preview_div').html(data.content);
                    $('#digital_signature_preview').modal('show');
    ...
    function appmails_preview() {
        let ascid = $('#ch_asso_id').val();
        let sign = $('#signature64').val();
        $('#sign_error').hide();
        let sign_type = $('input[name="signature_type"]:checked').val();
        let sign_image = $('#sign_image_file').val();
        if (sign_type == 'digital_signature') {
            ...
            send_admin_sign_preview(ascid, sign);
        } else {
            // Allowing file type
            var allowedExtensions =
                /(\.jpg|\.jpeg|\.png|\.gif)$/i;
            ...
            send_upload_admin_preview(ascid, sign);
        }
    }

    function send_upload_admin_preview(assoc_id) {
        var file_data = $('#sign_image_file').prop('files')[0];
        var form_data = new FormData();
        form_data.append('file', file_data);
        form_data.append('assoc_id', assoc_id);
        $.ajax({
            url: base_url + 'offers/send_applicantmail_preview/',
            type: "post",
            data: form_data,
            processData: false,
            contentType: false,
    ...
    function send_admin_sign_preview(ascid, sign) {
        $.ajax({
            url: base_url + 'offers/send_applicantmail_preview/',
            dataType: 'json',
            type: 'POST',
    ...
    function app_accept(ascid) {
        console.log(ascid);
        bootbox.confirm({
            message: "Are you sure, move it to offer approved & ready to be send state?",
            buttons: {
                confirm: {
                    label: 'Yes',
                    className: 'btn-success'
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("click", ".FileLiSt", function() {
            ...
            $.post(base_url + 'file_manager/project_files/', {
                project_id: project_id
            }, function(data) {
                console.log(data);
                var html = '';
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreportclient", function() {
            ...
            // alert();
            $.post(base_url + 'reports/choose_clients', {
                branch_id: branch_id
            }, function(data) {
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreportemployee", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_projects', {
                branch_id: branch_id
            }, function(data) {
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreporttask", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_tasks', {
                branch_id: branch_id
            }, function(data) {
                var all_projects = JSON.parse(data);
                var html = '';
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreportemp", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_emp', {
                branch_id: branch_id
            }, function(data) {
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#all_departmentids", function() {
            ...
            // alert(branch_id);
            $.post(base_url + 'reports/choose_depart', {
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreportpayslip", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_payslipemp', {
                branch_id: branch_id
            }, function(data) {
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#branchreportattendance", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_attendance', {
                branch_id: branch_id
            }, function(data) {
                var all_employees = JSON.parse(data);
                var html = '';
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("change", "#dailybranch_id", function() {
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_dailyemp', {
                branch_id: branch_id
            }, function(data) {
                var all_employees = JSON.parse(data);
                var html = '';
...
<?php } ?>
<?php if ($uri_page == 'projects') { ?>
    <script>
        $(document).ready(function() {
            $(".select2-option.assign_users").select2({
