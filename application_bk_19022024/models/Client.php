...
class Client extends CI_Model
{
	...
	*/
	...
	static function get_all_clients_limit()
	{
		$ci =& get_instance();
		if($ci->session->userdata('user_id') != 1 && $ci->session->userdata('user_type_name') =='company_admin'){
			$branches  = self::$db->select('dgt_branches.*')->from('dgt_branches')->join('dgt_assigned_entities','dgt_branches.branch_id = dgt_assigned_entities.branch_id')->where('dgt_branches.branch_status','0')->where('dgt_assigned_entities.user_id',$ci->session->userdata('user_id'))->get()->result_array();
			...
			if(!empty($_GET['client_name'])){
				$cond_like['company_name'] = $_GET['client_name'];
			}
			if(!empty($_GET['client_email'])){
				$cond_like['company_email'] = $_GET['client_email'];
			}
			if(!empty($cond_like)){
				self::$db->like($cond_like);
			}
			return self::$db->where($where_arr)->where_in('branch_id',$branches)->limit($_GET['length'],$_GET['start'])->order_by('company_name','ASC')->get('companies')->result();
		 }
		 else{
			$where_arr 	=	array();
			$where_arr['is_lead']	=	0;
			...
			if($ci->session->userdata('user_id') ==1){
				return self::$db->where($where_arr)->limit($_GET['length'],$_GET['start'])->order_by('company_name','ASC')->get('companies')->result();
			}
			else{
				$where_arr['branch_id']	=	$$ci->session->userdata('user_id');
				return self::$db->where($where_arr)->limit($_GET['length'],$_GET['start'])->order_by('company_name','ASC')->get('companies')->result();
			}
		 }
	...
	static function view_by_branch_id($branch_id = NULL)
	{
		$ci =& get_instance();
		if($branch_id) {
			return self::$db->where(array('is_lead' => 0,'branch_id'=>$branch_id))->order_by('company_name','ASC')->get('companies')->result_array();
		} else {
			if($ci->session->userdata('user_id') ==1){
				return self::$db->where(array('is_lead' => 0))->order_by('company_name','ASC')->get('companies')->result_array();
			}
			else{
				return self::$db->where(array('is_lead' => 0,'branch_id'=>$ci->session->userdata('branch_id')))->order_by('company_name','ASC')->get('companies')->result_array();
			}
		}		
	}
