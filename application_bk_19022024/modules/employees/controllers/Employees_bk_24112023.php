...
function grid_employees()
    ...
    // function employees_bulk_update_bk($postData, $files)
    // {
    //     set_time_limit(0);
    ...
    //                 {
    //                      $this->db->or_where("dgt_users.email", trim($row[2]));
    //                 }
    ...
    //                     $this->db->from("dgt_branches");
    //                     //$this->db->where("branch_prefix", trim($row[8]));
    //                     $this->db->where("branch_name", trim($row[8]));
    ...
    //                     $branch_results = $branch_query->result_array();
    //                     $branch_id = 0; 
    //                     if(!empty($branch_results))
    //                     {
    //                         foreach($branch_results as $branch_results_val)
    //                         {
    //                             $branch_id = $branch_results_val['branch_id'];
    //                         }
    //                     }
                        
    //                     $designation_id = $department_id  = 0;

    //                     if(isset($row[4]) && $row[4] != "")
    //                     {
    //                         $this->db->select("*");
    //                         $this->db->from("dgt_designation");
    //                         $this->db->where("dgt_designation.designation", $row[4]);
    //                         $this->db->group_by("id");
    ...
    //                             {
    //                                 $designation_id = $designation_query_val['id'];
    //                                 $department_id = $designation_query_val['department_id'];
    //                             }
    //                         }
    //                         else{
    ...
    //                         $this->db->from("dgt_account_details");
    //                         $this->db->join("dgt_users","dgt_users.id = dgt_account_details.user_id","inner");
    //                         $this->db->where("dgt_users.email", trim($row[6]));
    //                         $this->db->group_by("dgt_users.id");
    //                         $reporting_manager_query = $this->db->get();
    //                         if(!$reporting_manager_query)
    //                         {
    //                             return false;
    //                         }
    //                         $reporting_manager_result = array();
    //                         $reporting_manager_result = $reporting_manager_query->result_array();

    //                         $reporting_manager_id = '0';
                            
    //                         if(!empty($reporting_manager_result))
    //                         {
    //                             foreach($reporting_manager_result as $reporting_manager_val)
    //                             {
    //                                 $reporting_manager_id = $reporting_manager_val['user_id'];
    //                             }
    //                         }
    //                     }
    ...
    //                         "department_id"     =>  $department_id,
    //                         "teamlead_id"       =>  $reporting_manager_id,
    //                         "is_teamlead"       =>  'yes',
    ...
    //                     foreach ($emp_user_id as $emp_id_val) {
    //                         $emp_id =  $emp_id_val['user_id'];
    //                     }

    //                     $this->db->where("id",$emp_id);
    //                     $this->db->update("dgt_users", $user_data);
                        
    //                     $temp_employee_id_exp = array(); 
                        
    //                     $temp_employee_id_exp = explode($row[8], $row[0]);
    //                     $temp_employee_id = '';
    ...
    //                         "reporting_to2" => $reporting_manager_id,
    //                         "employee_id"   => $temp_employee_id,
    //                         "branch_id"     => $branch_id,
    //                         "doj"           => date("Y-m-d H:i:s", strtotime($row[9])),
    //                         "emp_gjd"       => date("Y-m-d H:i:s", strtotime($row[10])),
    //                      );
        ...
        if ($this->upload->do_upload('import_csv')) {
            ...
            if($file) 
            {
                ...
                while (($row = fgetcsv($file)) !== false) 
                {
                    ...
                    if(isset($row[0]) && $row[0] != "" && isset($postData['action_by']) && $postData['action_by'] == '1')
                    {
                        $this->db->where("dgt_account_details.emp_code", trim($row[0]));
                    }
                    else{
                    ...
                    if(!empty($emp_user_id))
                    {
                        ...
                        if(isset($row[6]) && $row[6] != "")
                        {
                            ...
                            if(!empty($designation_result))
                            {
                                foreach($designation_result as $designation_query_val)
                                {
                                    ...
                                    $emp_department = $designation_query_val['department_id'];
                                }
                            }
                            else
                            {
                        ...
                        if($row[32] != "")
                        {
                             $emp_bank_info['bank_ac_no'] = trim($row[32]);
                        }
