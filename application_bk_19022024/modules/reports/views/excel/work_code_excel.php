...
date_default_timezone_set('Asia/Kolkata');
  ...
  $punch_in_date_time = date('Y-m-d H:i');
   ...
   $subdomain_id     = $this->session->userdata('subdomain_id');
        ...
        //$employee_incidents =  $this->db->get('calendar_incident',array('subdomain_id'=>$subdomain_id))->result_array();
        if($branch_id != '') {
            $this->db->where("u.branch_id IN (".$branch_id.")",NULL, false);
        }
  ...
  <table id="table-late_arrival_report" class="" style="vertical-align: middle !important;text-align: center; border-collapse: collapse;width: 100%;border: 1px solid #3a3a3a; padding: 8px;">
    ...
    <tbody>
     ...
     <?php
                ...
                $a_year =date('Y', $time);
                 ...
                 if(!empty($rows['month_days'])){
                  ...
                  if(!empty($day['punchout_workcode']) || !empty($day['punchin_workcode'])){
                    ...
                    $punch_workcode =  $this->db->get_where('incidents',array('id' => $work_code))->row_array();                                       
        ...
        foreach ($workcode_id as $key => $value) { 
          $workcode_count[$value][] = $key; 
        }
        foreach ($workcode as $key => $value) {?>
          <tr class="attendance_record" style="vertical-align: middle !important;">
                <td style="vertical-align: middle !important;text-align: center;"><?php echo  $value ?></td>
                <td style="vertical-align: middle !important;text-align: center;" ><?php $incident_name =  $this->db->get_where('incidents',array('id' => $value))->row_array();
                      echo ucfirst($incident_name['incident_name']); ?></td>
                <td style="vertical-align: middle !important;text-align: center;"><?php echo (isset($workcode_count[$value]))?count($workcode_count[$value]):0;?></td>
          </tr>
        <?php } ?>
    </tbody>
  </table>