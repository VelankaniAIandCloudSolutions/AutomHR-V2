...
export class MyApp {
  ...
  checkData() {
    let data = [];;
    console.log(data);
    data = JSON.parse(localStorage.getItem("OFFLINE_DATA"));
    if (data) {
      data.forEach((element, key) => {

        if (element.punch == "punch_in") {
          this.token = localStorage.getItem('token');
          var token = { "token": this.token };
          var postData = {
            latitude: element.latitude,
            longitude: element.longitude,
            punch_date: element.punch_date
          }
          this.url = this.apiService.offlinePunchIn();
          ...
          fileTransfer.upload(element.pictureUri, this.url, options1)
            .then((res) => {
              this.referenceService.basicAlert('New HRMS','Attendance Synced to Server');
              console.log(res);
          ...
          this.url = this.apiService.offlinePunchOut();
          const fileTransfer: FileTransferObject = this.transfer.create();
          let options1: FileUploadOptions = {
            fileKey: 'user_profile_pic',
            fileName: 'image.jpg',
